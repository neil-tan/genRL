#!/usr/bin/env python
"""
Test script to verify the RecordVideoWrapper works correctly.
...
# Update import to use the new wrapper class and filename
...
def test_env_video(env_id, num_steps=50, frame_width=None, frame_height=None):
    ...
    # Apply video wrapper (using the new class name)
    # Note: This script tests the single-process scenario, so no primitives needed.
    video_env = RecordVideoWrapper(
        env=env,
        name_prefix=f"test-{env_id}",
        video_length=num_steps,  # Record all steps
        fps=10,
        record_video=True,
        # Use default episode trigger (record first episode) for simplicity in this script
