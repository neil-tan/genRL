...
import numpy as np
import wandb
from genRL.tasks.cartpole import grpo_training_loop
...
def main():
    args = tyro.cli(
                ...
                default=SessionConfig(
                    ...
                    ppo=GRPOConfig(num_envs=32, n_epi=180),
                ),
    ...
    if get_platform() == "macOS" and sys.gettrace() is None:
        gs.tools.run_in_another_thread(fn=grpo_training_loop, args=(env, config, run))
    else:
        grpo_training_loop(env, config, run)
